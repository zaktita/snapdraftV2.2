...
class GoogleGeminiService implements AIServiceInterface
{
    ...
    // Text-to-image model (Imagen) — not used with generateContent
    ...
    public function generateWithReferences(
        ...
        try {
            ...
            if (!$response->successful()) {
                Log::error('Gemini API error during image generation', [
            ...
            Log::error('No image data in Gemini response', [
                'model' => $modelToUse,
    ...
    protected function selectModel(bool $hasReferences, bool $textAccurate): string
    {
        ...
        // Priority 2: No references → still use the Gemini image-capable model.
        // Imagen models are not compatible with the v1beta generateContent endpoint used here.
        if (!$hasReferences) {
            Log::info('No references provided; using image model', ['model' => $this->imageModel]);
