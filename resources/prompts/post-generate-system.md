...
5. **Do not change language** when rewriting — French in → French out, Arabic in → Arabic out, etc.
...
2. Rewrite the caption in the **same language as the input** → condensed `on_image_text` (key facts per zone) + concise rewritten `post.caption` in brand voice.
...
5. `quality.avoid` must block off-brand failure modes, wrong-cluster drift, **text-heavy layouts** (full caption on image, paragraphs, speech bubbles, long bullet lists), and **wrong-language copy** (e.g. English on-image text when the caption is French).
