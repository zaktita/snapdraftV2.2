...
class JsonPromptCompiler
{
    ...
    public function buildImageRequestPrompt(
        ...
        if ($referenceImageCount > 0) {
            ...
            $parts[] = 'Use references for WHERE each text type goes (headline area, subhead, pills, lists, CTAs) and how long each zone typically is. Do NOT copy any words from the references. Do not invent a new grid, collage, speech bubble, or modular layout.';
        }
        ...
        if ($caption !== '') {
            $parts[] = 'Social caption (for fact-checking only — do NOT render this full text on the image; use only the ON-IMAGE TEXT strings above, each in its assigned zone):'
                ."\n\"{$caption}\"";
        }
    ...
    protected function formatOnImageTextBlock(mixed $onImageText, bool $useReferenceZones = false): string
    {
        ...
        return 'ON-IMAGE TEXT — spell exactly, character for character. '
            .$zoneHint
            ."One short string per zone — never the full social caption, never a paragraph or speech bubble. Do not copy reference wording:\n"
            .implode("\n", $numbered);
    }

    /**
     * Flatten a brand post-prompt JSON into a rich natural-language prompt for image models.
     * Tolerates minor schema drift from the LLM (must_have vs include, nested on_image_text, etc.).
     *
     * @param  array<string, mixed>  $promptJson
     */
